...
document.addEventListener("DOMContentLoaded", function() {
  ...
  const fontPath = "https://use.typekit.net/apb7wfu.css";
  const localCssPath = "sib.css";
  let cssPromise = fetch(cssPath).then(response => response.text());
  let fontPromise = fetch(fontPath).then(response => response.text());
  let stylePromises = [cssPromise, fontPromise];
...
`;
  ...
  Promise.all(stylePromises).then(styles => {
    ...
    s.type = "text/css";
    s.innerHTML = styles[0] + customCustomCSS;
    document.head.appendChild(s);
    let f = document.createElement("link");
    f.rel = "stylesheet";
    f.href = "https://use.typekit.net/apb7wfu.css";
